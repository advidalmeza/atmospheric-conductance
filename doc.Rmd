...
## Objective
For a given forest, you will perform a sensitivity analysis of model predictions of conductance. Atmospheric conductance refers to how easily vapor diffuses from vegetation surfaces.  

Consider the sensitivity of your estimate to uncertainty in the following parameters and inputs: 
- $h$ is vegetation height (cm)
- $k_d$ is a parameter; default 0.7
- $k_0$ is a parameter; default 0.1
- $v$ is windspeed (cm/s)

### Assumptions
- Assume windspeed, $v$, is normally distributed with a mean of 250 cm/s; a standard deviation of 30 cm/s
- Assume vegetation height, $h$, is somewhere between 9.5 and 10.5 m (but any value in that range is equally likely)
- Assume the $k_d$ and $k_0$ are normally distributed; standard deviation of 1% of their default values


## Latin Hypercube Sampling
Let's use the Latin hypercube approach to generate parameter values for the parameters and inputs:
```{r}
...
Let's now run the atmospheric conductance model for these parameters and inputs:
```{r}
...
## Plots of Parameter versus
Let's plot conductance estimates in a way that accounts for parameter uncertainty:
```{r warning = FALSE}
...
## Discussion 
**What do your results tell you about how aerodynamic conductance? What does it suggest about what you should focus on if you want to reduce uncertainty in aerodynamic conductance estimates? Does this tell you anything about the sensitivity of plant water use to climate change?**
Aerodynamic conductance is most sensitive to the inputs windspeed, $v$, which is normally distributed, and vegetation height, $height$, which is uniform distributed. We can improve uncertainty in aerodynamic conductance estimates by focusing on either input. This is a consideration for climate change since wind speed will vary with regional and seasonal variability and plant water use is especially sensitive. 
